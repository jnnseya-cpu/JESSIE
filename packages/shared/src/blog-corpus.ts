 ...
 * The article prose.
 *
 * This lived in the frontend, deliberately: the API had no reason to carry
 * it and shipping it to the backend bundle looked like dead weight in a
 * container. That reasoning was sound right up until something server-side
 * needed to read an article, and then it produced a bug nobody could see.
 *
 * The SEO audit scores a draft's body. The backend seeded these articles
 * with `body: ''` because the prose was over here, so every audit of a
 * hand-written post answered "not audited" — and since the whole published
 * corpus is hand-written, no article on the site had an SEO score at all.
 * The scores were not broken; there was nothing to score.
 *
 * So the corpus lives in shared now, where the site renders it and the
 * audit can read it, and there is one copy of it rather than one copy and
 * one silence. It is roughly forty kilobytes of prose next to a node_modules
...
/**
 * The article as the SEO auditor reads it.
 *
 * The site renders structured sections; the auditor counts words, headings
 * and links in a markdown-ish string. This is the one translation between
 * the two, so a score is computed from exactly the prose a reader sees
 * rather than from a summary of it.
 *
 * Headings are emitted as `##` because that is what the auditor's heading
 * rule looks for, and it is also what the article actually renders as — an
 * `h2` under the page title.
 */
export function articleBody(article: Article): string {
  const parts: string[] = [article.lede];
  for (const section of article.sections) {
    parts.push(`## ${section.h}`);
    for (const paragraph of section.p) parts.push(paragraph);
  }
  for (const link of article.links) parts.push(`[${link.label}](${link.href})`);
  return parts.join('\n\n');
}

/** The prose for a slug, or an empty string when the slug is not ours. */
export function bodyForSlug(slug: string): string {
