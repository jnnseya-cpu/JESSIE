import { GAME_WORLDS, GRACE_TOKENS_PER_MONTH, POINTS_NEVER } from '@jessmove/shared';
...
export function computeRewards(
  ...
  const completions = rows.filter((r) => r.kind === 'snap_completed');
  const offers = rows.filter((r) => r.kind === 'snap_offered');

  if (offers.length > 0) add('starting', offers.length * POINTS.started);
  if (completions.length > 0) add('completing', completions.length * POINTS.completed);
  ...
  for (const day of series) {
    if (day.completed === 0) lapsed = true;
    else if (lapsed) {
      returns += 1;
      lapsed = false;
    }
  ...
  // of the window on a weekday hour the member had never used before.
  const half = Math.floor(series.length / 2);
  const earlyHours = new Set(
    rows
      .filter((r) => r.kind === 'snap_completed' && r.onDay < (series[half]?.day ?? ''))
