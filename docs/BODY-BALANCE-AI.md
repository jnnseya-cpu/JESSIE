...
This module is specified against **Jess Move AI OS**, not JESS MOVE, and it collides
head-on with a shipped, CI-enforced guardrail.

**JESS MOVE Ethical Gamification Charter, rule C6:**
...
pipeline. That is deliberate — it is the mechanism §13.6.8 of the JESS MOVE specification
requires.
...
| Option | Consequence |
|---|---|
| **A — Separate product** | Body Balance ships as its own product, outside JESS MOVE. The Charter is untouched. No code change to the existing build. |
| **B — Amend C6** | JESS MOVE absorbs Body Balance. C6 must be rewritten and `charter.test.ts` amended with it. Under §6 Rule M0 this is a change of product category and needs documented, named sign-off. |
...
> These prohibitions are substantively the same instinct as JESS MOVE Charter C6. They are
...
> Reconciliation note: JESS MOVE §25.2 targets a 66% blended gross margin. The 4× rule
...
Note this is the same shape as the JESS MOVE prescription scoring function (§10.2), where
...
5. Relationship to the JESS MOVE six-mode model — Body Balance pathways and age modes are
...
6. Whether the safety-escalation agent constitutes a medical-device function under UK MDR.
   This is the same class of question as JESS MOVE open decision **O3**, and would gate
